...
class Food(models.Model):
    ...
    category = ManyToManyField(
        'Category', help_text="Enter the foods category e.g(milk)")
    ...
    def display_category(self):
        """displays all ManyToManyField category in a string"""
        return ', '.join(category.category_type for category in self.category.all())
    # default description for category fields in admin
    display_category.short_description = 'Categories'


class Category(models.Model):
    """Model representing food categories"""
    category_type = CharField(max_length=200, unique=True,
                              help_text='Enter the category type e.g (keto,vegan)', db_index=True)
    description = TextField(
        help_text='Enter summary on category type: e.g(vegan: health benefits, environmental impact)')
...
class StockFood(models.Model):
    ...
    stock_instance = ForeignKey(
        'StockInstance', on_delete=models.CASCADE, db_index=True)
    food = ForeignKey(Food, on_delete=models.CASCADE)
    quantity = PositiveSmallIntegerField(default=1)
    # remember to exclude in forms
    added_date = DateTimeField(auto_now_add=True)
    expiry_date = DateTimeField(
        help_text='Enter the expiry date (optional)', blank=True, null=True)
    ...
    def save(self, *args, **kwargs):
        """Save method overided to compute fields"""
        ...
        if self.expiry_date is None:
            ...
            # optimised query to just save()
            super().save(update_fields=['expiry_date'])
        # update food verification status
        self.update_food_verification(self.food)

    @property
    def expired(self):
        """determines if food has expired"""
...
class Recipe(models.Model):
    ...
    ingredients = ManyToManyField(
        Food, help_text='Enter the recipes ingredients')
    category = ManyToManyField(
        Category, help_text='Enter the dietary category e.g Vegan, Gluten Free')
    serving_sizes = (
        ('g', 'grams'),
        ('kg', 'kilograms'),
        ('oz', 'ounces'),
    ...
    def display_ingredients(self):
        """displays ManyToManyField ingredients in one continious string"""
        return ', '.join(food.name for food in self.ingredients.all())

    def display_category(self):
        """displays ManyToManyField category in one continious string"""
        return ', '.join(category.category_type for category in self.category.all())
